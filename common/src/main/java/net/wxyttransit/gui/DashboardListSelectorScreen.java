...
public class DashboardListSelectorScreen extends ScreenMapper implements IGui {
	...
	public DashboardListSelectorScreen(Runnable onClose, List<NameColorDataBase> allData, Collection<String> selectedIds, boolean isSingleSelect, boolean canRepeat) {
	...
	private DashboardListSelectorScreen(ScreenMapper previousScreen, Runnable onClose, List<NameColorDataBase> allData, Collection<String> selectedIds, boolean isSingleSelect, boolean canRepeat) {
		...
		selectedList = new DashboardList(null, null, null, this::updateList, null, this::onDelete, () -> selectedIds instanceof ArrayList ? (List<String>) selectedIds : new ArrayList<>(), () -> ClientData.ROUTES_PLATFORMS_SELECTED_SEARCH, text -> ClientData.ROUTES_PLATFORMS_SELECTED_SEARCH = text);
	...
	public void render(GuiGraphics g, int mouseX, int mouseY, float delta) {
		try {
			PoseStack matrices=g.pose();
			renderBackground(g);
			availableList.render(g, font);
			selectedList.render(g, font);
			super.render(g, mouseX, mouseY, delta);
			g.drawCenteredString(font, Text.translatable("gui.mtr.available"), width / 2 - PANEL_WIDTH / 2 - SQUARE_SIZE, SQUARE_SIZE + TEXT_PADDING, ARGB_WHITE);
			g.drawCenteredString(font, Text.translatable("gui.mtr.selected"), width / 2 + PANEL_WIDTH / 2 + SQUARE_SIZE, SQUARE_SIZE + TEXT_PADDING, ARGB_WHITE);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void onDelete(NameColorDataBase data, int index) {
		if (canRepeat && selectedIds instanceof ArrayList) {
			((ArrayList<String>) selectedIds).remove(index);
		} else {
