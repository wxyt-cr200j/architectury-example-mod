...
DashboardList implements IGui {
	...
	public void render(GuiGraphics g, Font textRenderer) {
		g.drawCenteredString(textRenderer, String.format("%s/%s", page + 1, totalPages), x + SQUARE_SIZE * 2, y + TEXT_PADDING + TEXT_FIELD_PADDING / 2, ARGB_WHITE);
		...
		for (int i = 0; i < itemsToShow; i++) {
			if (i + itemsToShow * page < dataFiltered.size()) {
				...
				g.drawString(textRenderer, drawString, 0, y + drawY, ARGB_WHITE);
				matrices.popPose();
			}
		}
	}
